...
import performance.com.example.ndh.mvvmdemo.net.NetApi;
import performance.com.example.ndh.mvvmdemo.net.RetrofitManager;
    ...
    private void requestNews(String url) {
        RetrofitManager.getRetrofit(mContext.getApplicationContext(), url).create(NetApi.class).requestNews(1, 20, 1)
                ...
                .map(new Function<NetResult, List<NetResult.NewsBean>>() {
                    ...
                    @Override
                    public List<NetResult.NewsBean> apply(NetResult netResult) throws Exception {
                        if (null != netResult)
